...
function makeLimit({ windowMs, max, message, skipSuccessful = false }) {
  return rateLimit({
    ...
    keyGenerator(req) { return `${req.tenant?.id ?? 'global'}:${req.ip ?? 'unknown'}`; },
...
const generalApiLimiter       = makeLimit({ windowMs: 60 * 1000,      max: 200, message: 'Too many requests. Please slow down.' });
