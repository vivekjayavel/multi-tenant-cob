...
exports.getBySlug = async (req, res, next) => {
  try {
    const [rows] = await db.query('SELECT id, name, description, price, image_url, images, category, slug, stock_qty, reserved_qty, customization_options, delivery_time, sort_order, created_at, updated_at FROM products WHERE tenant_id = ? AND slug = ? AND is_active = 1 LIMIT 1', [req.tenant.id, req.params.slug]);
...
exports.create = async (req, res, next) => {
  try {
    ...
    const { name, description, price, image_url, images, category, stock_qty = 0, customization_options, delivery_time, sort_order = 0 } = req.body;
    ...
    const [result] = await db.query('INSERT INTO products (tenant_id, name, description, price, image_url, images, category, slug, stock_qty, customization_options, delivery_time, sort_order) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [tenantId, name, description, price, image_url, imagesJson, category, slug, stock_qty, custOpts, delivery_time || null, parseInt(sort_order) || 0]);
...
exports.update = async (req, res, next) => {
  try {
    ...
    const allowed = ['name','description','price','image_url','images','category','slug','is_active','stock_qty','customization_options','delivery_time','sort_order'];
    const fields = [], values = [];
